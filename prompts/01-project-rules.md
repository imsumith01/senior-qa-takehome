I'm building a Senior QA Engineer take-home exercise. You will do all of the work.

Before we start, I want you to write a CLAUDE.md at the repo root containing the rules
below, so they persist across every session. Then confirm you've read them back to me.
...
The repository must be created fresh and should be public for this exercise. Do not clone, fork, copy, or
adapt any existing automation framework. Libraries and official docs are fine as
reference; the structure and code must be written here.
...
For each step I give you, in this exact order:
  1. Save my verbatim prompt to `prompts/NN-short-slug.md` (NN = zero-padded step number).
     Copy it exactly as I typed it, including any typos. Do not paraphrase or clean it up.
  2. Do the work.
  3. `git add -A`
  4. Write the commit message to a temp file, then commit with `git commit -F <file>`.
     ...
     Format:
       ...
       ---
       <the full prompt text I sent you, verbatim>
       ---

  5. `git push`

If a step is large, make several smaller commits instead of one giant one. Each of those
commits repeats the same verbatim prompt in its body and adds a line saying which slice of
the step it covers.

Never combine two of my prompts into one commit. Never commit without pushing.

=== RUNNING LOG ===
Maintain `docs/AI_EVALUATION_LOG.md` from the very first step. Every time you get
something wrong — a selector that didn't exist, an assumption about the API that turned
out false, a test that passed for the wrong reason, code you had to rewrite because it
was unreadable — append a dated entry at that moment with: what you produced, why it was
...
accessibility tree, read real attributes, confirm behaviour. Every selector you commit
must be one you actually observed through MCP, not one you recalled from training data.
If you find yourself writing a selector from memory, stop and go look.
...
=== TONE ===
When you disagree with something I've asked for, say so before doing it. When you're
guessing, say you're guessing. Do not tell me something works until you've run it.

Confirm you understand, write CLAUDE.md, commit it, and stop.
