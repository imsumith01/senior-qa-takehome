...
| Performance/latency assertions                                         | Shared public demo over the public internet — response time measures the network path, not the app. `performance_glitch_user` is covered functionally (login completes, WEB-008) with no timing threshold; the 5 s figure lives in the discovery doc.                                                                                                                                                                                                                                                                              |
...
| Checkout with an empty cart                                            | Behaviour was not observed during discovery, so there is no verified expectation to encode. Listed as a traceability gap; would be a five-minute exploratory check before automating.                                                                                                                                                                                                                                                                                                                                              |
...
several tags. IDs are stable and appear in test titles.
...
became parameterized, and the empty-cart checkout idea was cut in §1. IDs are never
reused.)
...
| Checkout with empty cart                                           | —                                                       | **gap**, unobserved behaviour (§1)                                                                                                                   |
