...
- **API (jsonplaceholder.typicode.com)**: the read contract of all six collections
  as plainly consumed (no query modifiers — see the out-of-scope row for the
  json-server query surface); single-resource and error semantics; nested-route/
  filter equivalence; the write _response_ contract and non-persistence, exercised
  against `/posts` as the representative resource; degenerate input handling; the
  safe subset of headers.
...
| Exhaustive bug matrix for `problem_user`, `error_user`, `visual_user`                            | These users are intentional, permanent fixtures — there is no developer who will fix them, so a full per-bug suite is maintenance without information. Three positive pins are implemented (WEB-031..033); three more are planned but unimplemented (WEB-024..026); the complete catalogue of their bugs is in the discovery doc and docs/KNOWN_DEFECTS.md.                                                                                                                                                                        |
...
| jsonplaceholder's query surface (`_page`, `_limit`, `_sort`, `_order`, `_embed`, `_expand`, `q`) | Never probed in discovery, so there is no observed contract to encode — and pagination is arguably the most consumer-relevant read feature, which makes this a real hole, not a shrug. Named here (self-review finding) rather than silently absent; probing it properly is a discovery pass of its own.                                                                                                                                                                                                                           |
...
| WEB-008 | performance_glitch_user completes login and sees the inventory (no timing assertion — proves the suite's waits, not the site's speed). UNIMPLEMENTED — deferred; the user is covered descriptively in discovery §2 | P0       | @regression               |
...
| WEB-012 | each sort option orders products correctly (four explicit tests, deliberately unrolled per readability rule 15; price ties compared monotonically, never by position)                                              | P2       | @regression               |
...
| WEB-017 | cart contents survive logout and re-login (pins observed persistence behaviour). UNIMPLEMENTED — deferred; behaviour documented in discovery §6                                                                    | P2       | @regression               |
| WEB-018 | Reset App State empties the cart; buttons resynchronize after reload (pins the stale-button behaviour honestly). UNIMPLEMENTED — deferred; behaviour documented in discovery §9                                    | P2       | @regression               |
...
| WEB-024 | defect pin: problem_user sees the 404-dog image on all six products. UNIMPLEMENTED — deferred; WEB-031 covers this user's cart defect                                                                              | P2       | @regression               |
| WEB-025 | defect pin: error_user add-to-cart leaves the cart unchanged for Bolt T-Shirt, Fleece Jacket, and Test.allTheThings(). UNIMPLEMENTED — deferred; WEB-032 covers this user's deepest defect                         | P2       | @regression               |
| WEB-026 | defect pin: visual_user's Test.allTheThings() button carries the btn_inventory_misaligned class. UNIMPLEMENTED — deferred; behaviour documented in discovery §2                                                    | P2       | @regression               |
| WEB-027 | cart page lists exactly the added items with their names, descriptions, prices, and quantities                                                                                                                     | P1       | @regression               |
| WEB-028 | removing an item on the cart page updates both the row list and the badge                                                                                                                                          | P1       | @regression               |
| WEB-029 | Continue Shopping returns to the inventory with the cart intact                                                                                                                                                    | P1       | @regression               |
...
| WEB-031 | defect pin, positive form: problem_user clicks the dead Bolt T-Shirt add button and the badge provably does not change — passes while the defect exists, fails red on a site-side fix or any breakage              | P2       | @known-defect @regression |
| WEB-032 | defect pin, positive form: error_user clicks Finish and provably stays on the overview — no order can ever complete; same alarm semantics as WEB-031                                                               | P2       | @known-defect @regression |
| WEB-033 | defect pin, positive form: problem_user's Last Name keystrokes land in the First Name field (the form-mangling defect, pinned separately so it flips independently of WEB-031)                                     | P2       | @known-defect @regression |
...
| API-002 | every collection returns its documented count and uniform shape (six explicit tests, deliberately unrolled per readability rule 15: the zod output types differ and unifying them costs more than the duplication) | P0       | @contract           |
...
| API-021 | cross-resource referential integrity: every post's userId corresponds to a real user                                                                                                                               | P1       | @contract           |
...
- **Hardcoded, in `src/data/` as named constants**: the six users and the shared
  ...
  customer details (static values; the app was observed rejecting empty fields and
  accepting one ordinary value set — anything beyond that is unverified); API
...
- 100% of implemented tests pass — P2 included. Six planned IDs are explicitly
  unimplemented (WEB-008, WEB-017, WEB-018, WEB-024..026, marked in the inventory)
  and are named gaps, not silent ones; no test is skipped silently (any skip
...
| ------------------------------------------------------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Login: success                                                     | WEB-001 (WEB-008 unimplemented)                                        | —                                                                                                                                                            |
...
| Cart persistence across logout                                     | WEB-017 (unimplemented)                                                | cross-user leakage variant not automated: inferred-only in discovery, and pinning user A's cart appearing for user B would bless a bug as a contract         |
...
| Known-defect users                                                 | WEB-024..026 (unimplemented), WEB-031..033 (implemented positive pins) | full bug catalogue deliberately unautomated (§1); Reset App State pin WEB-018 also unimplemented                                                             |
| Cart page contents and navigation                                  | WEB-027, WEB-028, WEB-029                                              | —                                                                                                                                                            |
| Checkout with empty cart                                           | WEB-030                                                                | gap closed: behaviour observed in a follow-up session (discovery §10.16) and pinned                                                                          |
| Defect-detection pins (problem_user, error_user)                   | WEB-031, WEB-032, WEB-033                                              | positive pins of the defective behaviour: pass while the defect exists, fail red on a site-side fix or any breakage — docs/KNOWN_DEFECTS.md has the register |
...
| API: cross-resource referential integrity                          | API-021                                                                | —                                                                                                                                                            |
...
| Keyboard submission (Enter in the login and checkout forms) | — | **gap** (self-review): the most common real-user gesture on a login form; needs the form's Enter behaviour observed before an expectation can be encoded |
| Cookie forgery (session-username set to garbage or locked_out_user) | — | **gap** (self-review): only the absent-cookie side of the guard is tested; the forged-cookie side needs live observation first |
| Add-to-cart from the product detail page as the acting path | — | **gap** (self-review): a distinct code path (generic add-to-cart/remove controls); WEB-016 only reads its state. Candidate test: open detail, add, assert badge, remove |
| Mid-checkout deep links while logged in (straight to step two / complete) | — | **gap** (self-review): unobserved; a step-skipping URL may bypass validation — the bug class error_user proves the app is prone to |
| Sort-state persistence across navigation | — | **gap** (self-review): WEB-012 asserts order only immediately after selection; unobserved beyond that |
| Detail-page guard and invalid ?id deep links | — | **gap** (self-review, restoring two dropped discovery flags): /inventory-item.html is absent from the WEB-009 sweep, and ?id=99 while logged in was never explored |
| API single-resource reads beyond /posts | API-004, API-005 | representativeness assumption now stated: single-item serialization and miss semantics are verified on /posts only; /users/{id} (the deepest shape) is untested |
